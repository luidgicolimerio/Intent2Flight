...
async def agente_roteamento(system_prompt: str, comando: str) -> str:
...
async def agente_telemetria(mcp_url: str) -> dict:
    ...
    resposta = await agente.ainvoke({"messages": [{"role": "user", "content": "Busque a posição e velocidade atual do drone via get_ned."}]})
    return resposta


async def agente_piloto(mcp_url: str, system_prompt: str, rota: str, comando: str) -> dict:
    client = MultiServerMCPClient({"uav": {"url": mcp_url, "transport": "http"}})
    todas_tools = await client.get_tools()
